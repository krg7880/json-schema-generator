...
describe('Generator', function() {
  ...
  it('should identify multiple property types in array of objects', function() {
    var data = fs.readFileSync(path.resolve(process.env.PWD) + '/test/fixtures/json/multiple-prop-types.json');
    data = JSON.parse(data.toString('utf8'));

    var schema = fs.readFileSync(path.resolve(process.env.PWD) + '/test/fixtures/schema/multiple-prop-types.json');
    schema = JSON.parse(schema.toString('utf8'));
